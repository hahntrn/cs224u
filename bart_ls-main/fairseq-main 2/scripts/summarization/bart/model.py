...
from .pooling_layer import PoolEncoderLayer
...
class BARTModel(TransformerModel):
    ...
    def __init__(self, args, encoder, decoder):
        ...
        self.encoder.layers.extend(
            [self.encoder.build_encoder_layer(self.encoder.cfg) for i in range(self.encoder.cfg.encoder.layers - 4)]
        )
        self.encoder.layers.extend(
            [PoolEncoderLayer(self.encoder.cfg) for i in range(4)]
        )
    ...
    def forward(
        ...
        #encoder_out = self.poolenclayer(encoder_out)
